...
@interface _InputBar() <UITextViewDelegate>
...
- (void)show {
    if([self.delegate respondsToSelector:@selector(inputBarWillShow:)]){
        [self.delegate inputBarWillShow:self];
    }
...
- (void)hide {
    ...
    if([self.delegate respondsToSelector:@selector(inputBarWillHide:)]){
        [self.delegate inputBarWillHide:self];
    }
